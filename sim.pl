:- initialization(open('in.fifo', read, _, [type(binary),alias(rd),eof_action(eof_code),buffering(none)])).
:- initialization(open('out.fifo', write, _, [type(binary),alias(wr),eof_action(eof_code),buffering(none)])).
:- initialization(main).

main :- exhaust(exchange, X, []), maplist(put_hex, X), nl.
put_hex(Byte) :- (Byte < 0x10 -> Padding = '0'; Padding = ''), format("~a~16R", [Padding,Byte]).

exchange -->
    getb([Cla,Ins,P1,P2,Lc]), { length(Dt, Lc) }, getb(Dt), getb([Le]),
    { once(response_for(Cla, Ins, P1, P2, Dt, Le, Rs, Sw1, Sw2)) },
    putb(Rs), putb([Sw1,Sw2]).

getb(Bytes) --> io(get_byte(rd), Bytes).
putb(Bytes) --> io(put_byte(wr), Bytes).
io(Method_1, Bytes) --> { maplist(good(Method_1), Bytes) }, Bytes.
good(G_1, Byte) :- call(G_1, Byte), Byte >= 0.

response_for(0x00, 0xA4, 0x04, 0x00, Dt, 0x00, Rs, 0x90, 0x00) :-
    Dt = [50,80,65,89,46,83,89,83,46,68,68,70,48,49], % 2PAY.SYS.DDF01
    Rs = [111,45,132,14,50,80,65,89,46,83,89,83,46,68,68,70,48,49,165,27,191,12,24,97,22,79,7,160,0,0,0,3,16,16,80,11,86,73,83,65,32,67,82,69,68,73,84].
response_for(0x00, 0xA4, 0x04, 0x00, Dt, 0x00, Rs, 0x90, 0x00) :- Dt = [0xA0,0x00,0x00,0x00,0x03,0x10,0x10],
    Rs = [111,50,132,7,160,0,0,0,3,16,16,165,39,80,11,86,73,83,65,32,67,82,69,68,73,84,159,56,12,159,102,4,159,2,6,95,42,2,159,55,4,191,12,8,159,90,5,0,8,64,8,64].
response_for(0x00, 0xA4, 0x04, 0x00, Dt, 0x00, [], 0x90, 0x00) :- Dt = [0xD2,0x76,0x00,0x00,0x85,0x01,0x00].
response_for(0x00, 0xA4, 0x04, 0x00, Dt, 0x00, [], 0x6A, 0x82) :- Dt = [0xD2,0x76,0x00,0x00,0x85,0x01,0x01].
response_for(0x00, 0xA4, 0x04, 0x00, Dt, 0x00, [], 0x6A, 0x82) :- Dt = [0xE1,0x03].
response_for(0x80, 0xA8, 0x00, 0x00, Dt, 0x00, Rs, 0x90, 0x00) :- Dt = [0x83,0x10,0x36,_,0x40,0,0,0,0,0,_,_,_,_,_,_,_,_],
    Rs = [119,61,87,16,71,97,115,144,1,1,1,25,210,65,34,1,23,88,148,114,130,2,0,0,95,52,1,1,159,16,7,6,1,17,3,160,0,0,159,38,8,19,201,29,101,169,16,201,86,159,39,1,128,159,54,2,0,2,159,108,2,128,0].
response_for(_, _, _, _, _, Le, Rs, 0x68, 0x00) :- length(Rs, Le).

exhaust(G_2, Curr, Last) :-
    phrase(G_2, Curr, Next) -> exhaust(G_2, Next, Last); Curr = Last.
