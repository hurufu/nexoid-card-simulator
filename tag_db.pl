...
t(tag_db, true, ('The most generic query must succeed at least once' :-
     once(tag_property_asn(_, _, _, _, _))
)).

t(tag_db, skip, ('There should exist a universal (31...) tag with 2 bytes length (known bug)' :-
    tag_property_asn(_, universal, _, _, 2)
)).

t(tag_db, skip, ('Tag 31 with 2 bytes serialization must exist (known bug)' :-
    tag_property_asn(_, universal, _, 31, 2)
)).
t(tag_db, skip, ('Numeric value serialization should find the smallest representation (known bug)' :-
